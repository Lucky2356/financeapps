...
export type TransactionRow = {
  ...
  tags?: string[];
  splitGroupId?: string;
  /** Когда операцию записали (ISO). Порядок операций внутри одного дня. */
  createdAt?: string;
  /**
...
export type PlanFactMonth = {
  ...
  /**
   * Итог доходов по двум группам счетов — у каждой свой план, факт и разница.
   *
   * План статьи делится по тому, куда её деньги ходят на самом деле: статья,
   * чьи деньги по истории оседают на сбережениях («Проценты по вкладу»), и
   * планом идёт туда же. Статья без истории — в основные.
   */
  incomePools: PlanFactPoolCells;
  /** То же для расходов. */
  expensePools: PlanFactPoolCells;
  /**
