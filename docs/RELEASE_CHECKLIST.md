...
## Раскатка: сначала себе, потом всем

Тег публикует выпуск **предварительным**. Это не формальность: обновлятор на ПК,
проверка на телефоне и запасной путь через API спрашивают у GitHub «последний
выпуск», а последним GitHub считает последний НЕ предварительный. Значит свежий
выпуск не видит никто — пока вы не решите иначе.

1. **Поставьте себе руками.** Со страницы выпуска: `.exe` на компьютер, `.apk` на
   телефон. Ссылка на предварительный выпуск открывается как обычно, он просто не
   считается последним.
2. **Поживите с ним.** Смоук-тест из этого файла плюс то, что менялось в выпуске.
   Столько дней, сколько считаете нужным, — никто в это время ничего не получает.
3. **Продвиньте.** Действие `Promote Release`, ввести тег. Оно ещё раз проверит,
   что манифест на месте и файлы, на которые он указывает, отдаются, — и сделает
   выпуск общим.

Продвижение **ничего не пересобирает**: всем уходят ровно те файлы, которые вы
проверяли на себе. Пересборка «из того же кода» дала бы другой установщик с
другой подписью, и проверенным он был бы только на словах.

Если выпуск оказался плохим — не продвигайте его. Он останется предварительным,
и для всех, кроме вас, его как не было. Следующий тег просто выпустит новый.

### Откат, если пробная сборка оказалась плохой

Схема хранилища едет только вперёд, и миграции необратимы: «сложить два поля в
одно» нельзя разложить обратно, потому что раскладывать уже нечего. Поэтому
откат устроен не через обратные миграции, а через копию.

**Приложение само откладывает книгу перед переводом на новую схему.** Копия
одна, снимается в ту единственную секунду, когда прежняя книга ещё существует, и
заменяется каждым следующим переводом. На сервер она не уезжает — это копия про
эту машину.

Вернуться так:

1. В приложении: **Импорт → Резервная копия → «Книга до последнего обновления» →
   Выгрузить файлом.** Блок появляется, только если перевод действительно был.
2. Поставить прежнюю версию.
3. Развернуть выгруженный файл обычным восстановлением из копии.

Блока нет — значит перевода не было, и прежняя версия откроет книгу как есть.

Отдельная резервная копия перед установкой пробной сборки всё равно не лишняя:
копия внутри приложения не переживёт удаления приложения или очистки хранилища.

### Чего этот порядок пока НЕ даёт

**Авто-доставки вам.** Предварительные сборки ставятся руками. Чтобы они
прилетали на ваши устройства сами, нужен переключатель канала в приложении —
отдельная работа, здесь её нет.
