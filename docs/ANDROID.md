...
Короткий путь — **код связки**. На устройстве, где данные уже есть:
**Настройки → Синхронизация → Мои устройства → «Связать ещё одно
устройство»**; появятся восемь знаков и картинка QR рядом. На телефоне — «У
меня есть код связки» → **«Навести камеру»**, и остаётся ввести свой пароль.
Камеру можно не давать: восемь знаков набираются руками, путь тот же.
...
приложение работает полностью. Адрес службы и имя входа приложение узнает
само. Код живёт пять минут и срабатывает один раз; пароля в нём нет и не
будет.

Длинный путь остаётся для тех, у кого своя служба (см. `server/README.md`):
**Настройки → Данные → Свой сервер → «У меня своя служба»**, адрес вводится
руками.

Служба хранит записи зашифрованными: ключа у неё нет и взяться ему неоткуда.
Что у вас есть учётная запись, как она называется и когда вы вносили правки —
видно тому, кто службу держит.
