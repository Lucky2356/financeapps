...
**Связать устройства.** Записи будут приезжать сами, в обе стороны. Нужна
служба синхронизации: своя (см. `server/README.md`) или чужая, к которой есть
доступ. На телефоне это делается прямо на первом запуске — «Данные уже есть на
другом устройстве», — или потом: **Настройки → Данные → Свой сервер**. Служба
хранит записи зашифрованными: ключа у неё нет и взяться ему неоткуда.
