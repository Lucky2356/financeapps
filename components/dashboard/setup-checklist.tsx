...
type Counts = { accounts: number; transactions: number; budgets: number; goals: number };
...
export function SetupChecklist() {
  ...
  const steps = [
    { done: counts.accounts > 0, title: "Добавьте счёт", desc: "Наличные, карта или накопительный", cta: "Открыть счета", href: "/accounts" as const },
    { done: counts.transactions > 0, title: "Запишите операцию", desc: "Доход или расход — или импортируйте CSV", action: "quick-add" as const },
    { done: counts.budgets > 0, title: "Задайте бюджет", desc: "Лимиты по категориям расходов", cta: "К бюджетам", href: "/budgets" as const },
    { done: counts.goals > 0, title: "Создайте цель", desc: "Накопления на крупную покупку", cta: "К целям", href: "/goals" as const }
  ];
    ...
    <div className="rounded-lg border border-primary/30 bg-primary/[0.06] p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Выполнено {doneCount} из {steps.length} — пройдите шаги, чтобы получить максимум.</p>
        </div>
          ...
          >
            <span
              className={cn(
                ...
                step.done ? "bg-success/20 text-success-foreground" : "bg-primary text-primary-foreground"
              )}
            >
            ...
            {!step.done ? (
              step.action === "quick-add" ? (
                <Button size="sm" variant="outline" onClick={() => window.dispatchEvent(new Event("quick-add-open"))}>
