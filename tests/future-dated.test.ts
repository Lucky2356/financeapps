...
describe("операции с датой в будущем", () => {
  // Граница проходит по концу сегодняшнего дня: вчера и сегодня — прошлое,
  // завтра — будущее. Три утверждения об одном правиле, поэтому и проверка одна.
  it("считает будущим только то, что после сегодняшнего дня", () => {
  ...
  it("сводит доходы и расходы в одно число со знаком", () => {
    ...
    expect(futureDated(ahead, NOW)).toEqual({ count: 2, net: 50000 });
    expect(futureDated([{ date: day("2026-08-01"), amount: 500, type: "EXPENSE" }], NOW)).toEqual({
      count: 0,
      net: 0
    });
  });
