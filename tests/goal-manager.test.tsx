...
vi.mock("next/navigation", () => ({ useRouter: () => ({ push: vi.fn(), refresh: vi.fn() }), usePathname: () => "/goals" }));
...
describe("GoalManager", () => {
  it("shows an empty state with a create CTA", async () => {
    render(<GoalManager data={emptyData} />);
  ...
  it("creates a goal through the dialog (calls the API and shows success)", async () => {
    ...
    await waitFor(() => expect(apiClientMock.post).toHaveBeenCalledWith("/goals", expect.objectContaining({ title: "Отпуск" })));
