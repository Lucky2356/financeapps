...
describe("синхронизирующее хранилище", () => {
  ...
  describe("повторный запуск", () => {
    it("не копит подписки: прежняя закрывается", async () => {
      // Приложение поднимает синхронизацию на каждом отпирании книги и ещё раз
      // сразу после подключения к службе. Раньше каждый заход открывал новый
      // поток, а прежний оставался висеть: ссылку перезаписывали, закрыть его
      // было уже нечем.
      //
      // Платили за это дважды. Каждое изменение прилетало столько раз, сколько
      // потоков накопилось, и столько же раз устройство шло за книгой — в
      // журнале службы это пачка одинаковых запросов на одну операцию. А на
      // телефоне за брошенные соединения платит батарея.
    ...
    it("остановка закрывает подписку", async () => {
      await storage.start(server, glue);
      storage.stop();
