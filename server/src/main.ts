...
export function forLog(value: string | undefined): string {
  ...
  // Обрезка по ЗНАКАМ, а не по единицам кода: срез посреди суррогатной пары
  // оставил бы в журнале половину знака.
  const short = [...value].slice(0, 300).join("");

  // Перевод строки и возврат каретки названы в наборе отдельно, хотя оба и так
  // попадают в диапазон рядом. Это не описка: ими дописывают в журнал
  // поддельные записи, то есть ровно ради них всё и затевалось, — и именно в
  // таком виде разбор кода видит, что строка обеззаражена. Диапазоны рядом
  // убирают остальное невидимое: забой, управляющие последовательности
  // терминала — всё, от чего строка в журнале выглядит не так, как её читают.
