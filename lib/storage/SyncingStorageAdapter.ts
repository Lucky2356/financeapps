...
/**
 * Копия книги, отложенная перед переводом на новую схему хранилища.
 *
 * Ключ объявлен ЗДЕСЬ, а не там, где копия делается, и это не мелочь. Тот, кто
 * заводит такой ключ, и тот, кто решает, что не уезжает на сервер, — разные
 * места в разных слоях, и разойтись им ничего не мешает. Общая постоянная
 * связывает их намертво: имя копии и правило «не отправлять» построены из
 * одного и того же.
 */
export const PRE_UPGRADE_SUFFIX = ":before-upgrade";

/**
 * Копия книги, отложенная, когда её не удалось прочитать.
 *
 * Уезжала на сервер до сих пор — и зря. Заводила себе там отдельную ячейку,
 * то есть хранила ВТОРУЮ книгу на каждое устройство, где что-то однажды
 * сломалось, и разносила её по остальным устройствам как ячейку, которой у них
 * нет. Копия эта — про поломку на конкретной машине, и место ей на этой машине.
 */
...
export class SyncingStorageAdapter implements StorageAdapter {
   ...
   *
   * Зовут это не один раз. Приложение поднимает синхронизацию на каждом
   * отпирании книги, а ещё сразу после подключения к службе — то есть за один
   * сеанс сюда заходят по нескольку раз. Раньше каждый заход открывал НОВЫЙ
   * поток событий, а прежний оставался висеть: ссылку на него просто
   * перезаписывали, закрыть его было уже нечем.
   *
   * Стоило это дорого и незаметно. Каждое изменение прилетало столько раз,
   * сколько потоков накопилось, и столько же раз шли за книгой — в журнале
   * службы это видно пачкой одинаковых запросов на одну операцию. На телефоне
   * за это платит батарея: висящие соединения никуда не деваются.
   */
  async start(transport: SyncTransport, merge: Merge): Promise<void> {
    // Прежний поток закрывается ПЕРВЫМ делом, до всего остального.
    this.unwatch?.();
    this.unwatch = null;
